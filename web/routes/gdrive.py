...
async def upload_package_to_drive(
    ...
    course_dir = PACKAGES_DIR / body.course
    if not course_dir.is_dir():
        raise HTTPException(404, f"패키지를 찾을 수 없습니다: {body.course}")
    ...
    course_folder = uploader.find_or_create_folder(body.course, packages_folder)
...
async def upload_downloads_to_drive(
    ...
    course_dir = DOWNLOADS_DIR / body.course
    if not course_dir.is_dir():
        raise HTTPException(404, f"다운로드 폴더를 찾을 수 없습니다: {body.course}")

    uploader = get_uploader()
    studyhub_folder = uploader.find_or_create_folder("StudyHub", GDRIVE_ROOT_FOLDER_ID)
    dl_folder = uploader.find_or_create_folder("크롤링", studyhub_folder)
    course_folder = uploader.find_or_create_folder(body.course, dl_folder)
